...
## 整点习题
### 3. 【全国联考 2012 年】
3. 假设某计算机按字编址，Cache 有 4 个行，Cache 和主存之间交换的块大小为 1 个字。若 Cache 的内容初始为空，采用 2 路组相联映射方式和 LRU 替换策略。访问的主存地址依次为 0，4，8，2，0，6，8，6，4，8 时，命中 Cache 的次数是（）。
A.1
B.2
C.3
D.4
...
### 5. 
<div style="border: 2px solid #ADD8E6; border-width: 2px 0 2px 0; padding: 10px; word-wrap: break-word; white-space: pre-wrap;">5．有效容量为 128 KB 的 Cache，每块 16 字节，8 路组相联。字节地址为 1234567 H 的单元调入该 Cache，其 Tag 应为。
A.1234 H 
B.2468 H 
C.048DH 
D.12345 H 
【解答】
cache 地址包含两大部分：块内地址和块号，其中块号位于高地址，包括组号和标记位，本题需要计算出组号所占位数，通过已知条件得出标记位的位数。
需要注意，题目虽然没有明确说明按字节还是按字编址，但是说“字节地址”的时候就提示了是按字节编址。用容量除以每块大小可以得到块数为 $2^10$，占 10 位，数据大小占 4 位，给定地址去掉低 14 位剩下就是标记位了。
</div>
